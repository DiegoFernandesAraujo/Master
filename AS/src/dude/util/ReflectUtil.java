...
public class ReflectUtil {
	...
	public static <T> T newInstance(Class<T> type, Object... params) throws IllegalArgumentException {
		try {
			...
			if (candidateDistances.isEmpty())
				throw new IllegalArgumentException(String.format("no suitable constructor found in %s for %s", type, Arrays.toString(params)));
			...
			if (bestConstructor == null)
				throw new IllegalArgumentException(String.format("more than one suitable constructor found in %s for %s", type, Arrays
						.toString(params)));
	...
	/**
	 * Returns the boxing class for the given primitive type. A primitive type returns true for {@link Class#isPrimitive()}.
	 * 
	 * @param primitive
	 *            the primitive type
	 ...
	 * Example: <code>class IntList extends ArrayList&lt;Integer&gt; {}</code> would result in a <code>BoundType.of(Integer.class)</code>.
	 * 
	...
	public static BoundType[] getStaticBoundTypes(ParameterizedType parameterizedType) {
		...
		if (rawType.getGenericSuperclass() instanceof ParameterizedType)
			boundedTypes.addAll(0, Arrays.asList(getStaticBoundTypes((ParameterizedType) rawType.getGenericSuperclass())));
	 ...
	 * A static bound occurs if a subclass specifies the type parameter of the superclass explicitly.<br>
	 * Example: <code>class Foo { Collection&lt;Integer&gt; bar; }</code> would result in a <code>BoundType.of(Integer.class)</code>.
	 * 
	 * @param field
	 ...
	 * Example: <code>class Foo&lt;T&gt; { Collection&lt;T&gt; bar; }; class IntFoo extends Foo&lt;Integer&gt; {}</code>.<br>
	...
	/**
	 * Resolves the {@link ParameterizedType} superclass for a given {@link BoundType}.<br>
	 * Example: <code>class Pair&lt;S, T&gt; { }; class OrderedPair&lt;X&gt; extends Pair&lt;X, X&gt; {}</code>.<br>
	...
	public static BoundType resolveParamizedSuperclass(BoundType type, ParameterizedType superclass) {
		...
		for (int index = 0; index < actualTypeArguments.length; index++) {
			if (actualTypeArguments[index] instanceof TypeVariable<?>)
				...
				arguments[index] = resolveParamizedSuperclass(type, (ParameterizedType) actualTypeArguments[index]);
			else
	...
	public static Object invoke(Object object, String function, Object... params) {
		...
		try {
			...
			if (candidateDistances.isEmpty())
				throw new IllegalArgumentException(String.format("no suitable method found in %s for name %s and parameters %s", type, function,
			...
			if (bestMethod == null)
				throw new IllegalArgumentException(String.format("more than one suitable method found in %s for name %s and parameters %", type,
						function, Arrays.toString(params)));
			return invoke(bestMethod, object, params);
		} catch (Exception e) {
			throw new IllegalArgumentException(String.format("Could not invoke method %s for type %s with parameters %s", function, type, Arrays
					.toString(params)), e);
		}
	}

	private static Object invoke(Method method, Object object, Object[] params) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
	...
	public static boolean hasFunction(Object object, String function, Object... params) {
		...
		try {
			...
			throw new IllegalArgumentException(String.format("Could not find method %s for type %s with parameters %s", function, type, Arrays
					.toString(params)), e);
		}
	}
	...
	public static Object getFieldValue(Object object, String fieldName) {
		...
		try {
			...
			throw new IllegalArgumentException(String.format("Could not get field value %s for type %s", fieldName, type), e);
		}
	}
}
