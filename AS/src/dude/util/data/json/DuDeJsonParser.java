...
import dude.exceptions.InvalidJsonException;
...
/**
 * <code>DuDeJsonParser</code> can be used for converting a String containing Json syntax into its Java representation. Besides providing methods for
 ...
 * {@link JsonArray}. <code>DuDeJsonParser</code> can be used for converting a Json String into its Java representation. Besides providing methods for
 * reading the data it also implements the {@link Iterator} interface. The iterating functionality can be used for parsing the content of a
 * {@link JsonArray}.
 * 
 ...
 */
public class DuDeJsonParser<T extends Jsonable> extends AbstractIterator<T> implements Closeable {

	private static final Logger logger = Logger.getLogger(DuDeJsonParser.class.getPackage().getName());
	...
	public static JsonValue getScalarValue(String jsonCode) {
		...
		try {
			final JsonParser parser = DuDeJsonParser.factory.createJsonParser(jsonCode);
			parser.nextToken();
			...
			DuDeJsonParser.logger.warn("Scalar value could not be converted into a JsonValue: " + jsonCode, e);
	...
	/**
	 * Initializes a new <code>DuDeJsonParser</code> using the passed {@link InputStream}.
	 * 
	 * @param type
	 *            The type that is generated during the iteration process. {@link JsonValue} instances are returned, if <code>null</code> is passed.
	 * @param stream
	 ...
	 */
	public DuDeJsonParser(Class<T> type, InputStream stream) throws JsonParseException, IOException {
		this.parser = DuDeJsonParser.factory.createJsonParser(stream);
		this.internalType = type;
		this.loadNextToken();
	}

	/**
	 * Initializes a new <code>DuDeJsonParser</code> using the passed {@link Reader}.
	 * 
	 * @param type
	 *            The type that is generated during the iteration process. {@link JsonValue} instances are returned, if <code>null</code> is passed.
	 * @param reader
	 ...
	 *             If an error occurred while reading from the stream.
	 */
	public DuDeJsonParser(Class<T> type, Reader reader) throws JsonParseException, IOException {
		this.parser = DuDeJsonParser.factory.createJsonParser(reader);
		this.internalType = type;
		this.loadNextToken();
	}

	/**
	 * Initializes a new <code>DuDeJsonParser</code> that parses the passed <code>String</code>.
	 * 
	 * @param type
	 *            The type that is generated during the iteration process. {@link JsonValue} instances are returned, if <code>null</code> is passed.
	 * @param str
	 ...
	 *             If an error occurred while reading from the stream.
	 */
	public DuDeJsonParser(Class<T> type, String str) throws JsonParseException, IOException {
	 ...
	 *             If an error occurred while reading from the stream.
	 */
	public DuDeJsonParser(InputStream stream) throws JsonParseException, IOException {
		this(null, stream);
	}
	 ...
	 *             If an error occurred while reading from the stream.
	 */
	public DuDeJsonParser(Reader reader) throws JsonParseException, IOException {
		this(null, reader);
	}
	 ...
	 *             If an error occurred while reading from the stream.
	 */
	public DuDeJsonParser(String str) throws JsonParseException, IOException {
	 ...
	 * The returned {@link DuDeJsonGenerator} might be used to generate arbitrary json elements in front of the current parser position in a FIFO
	 * manner.<br>
	 * In order to adjust the internal state to the newly generated json elements, a closing the returned generator with
	 * {@link DuDeJsonGenerator#close()} is necessary.
	 * 
	...
	public DuDeJsonGenerator createPushBackGenerator() {
		...
		return new DuDeJsonGenerator(this.pushBackGenerator) {
			@Override
			public void close() throws IOException {
				super.close();
				DuDeJsonParser.this.loadNextToken();
			}
		};
	}
	...
	protected String currentTokenName() {
		...
		try {
			...
			DuDeJsonParser.logger.warn("EOF: currentTokenName was called althoug the end of the content was reached.", e);
			return null;
		}
	}
